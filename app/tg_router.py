...
def make_router(bot_kind: str) -> Dispatcher:
    ...
    async def on_text(m: Message):
        ...
        try:
            ...
            logger.exception("note/amochats error: %s", e)
        ...
        if s.step <= 2:
            await m.answer(_survey_prompt(s.step))
        else:
            summary = _survey_summary(s.city, s.experience, s.time_pref)
            try:
                amo = await AmoClient.create()
            ...
            logger.info("[%s] survey finished user_id=%s lead_id=%s", bot_kind, m.from_user.id, s.lead_id)
